...
Child.prototype.createNewProcess = function createNewProcess() {
  ...
  this.process.on('exit', function(code) {
    ...
    if(self.retry >= 5) {
      console.log("Child process crash! Maximum number of attempts reached!");
      this.emit('error', new Error("Max retry for child crash"));
    }
    else {
      console.log("Child process crash! Retry!");

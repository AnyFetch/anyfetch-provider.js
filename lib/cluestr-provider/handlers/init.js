 ...
 * @param {Function} next Callback to call once res has been populated.
 */
exports.callback = function(connectAccountRetrievePreDatas, connectAccountRetrieveAuthDatas, cluestrClient, connectUrl) {
  return function (req, res, next) {
    async.waterfall([
      function(cb) {
        // Retrieve the previous tempToken
        connectAccountRetrievePreDatas(req, res, cb);
      },
      function(hash, cb) {
      ...
      function(tempToken, cb) {
        ...
        cluestrClient.getAccessToken(tempToken.cluestrCode, connectUrl, function(err, cluestrAccessToken) {
          cb(null, tempToken, cluestrAccessToken);
        });
      },
      function(tempToken, cluestrAccessToken, cb) {
        // Retrieve datas from provider
        connectAccountRetrieveAuthDatas(req, res, tempToken.datas, function(err, providerDatas, redirectUrl) {
          cb(err, tempToken, cluestrAccessToken, providerDatas, redirectUrl);
        });
      },
      function(tempToken, cluestrAccessToken, providerDatas, redirectUrl, cb) {
        // Create new token
        var token = new Token({
        ...
        token.save(function(err) {
          cb(err, tempToken, token, redirectUrl);
        });
      },
      function(tempToken, token, redirectUrl, cb) {
        tempToken.remove(function(err) {
          cb(err, token, redirectUrl);
        });
      }
    ], function(err, token, redirectUrl) {
      redirectUrl = redirectUrl || 'http://cluestr.com';

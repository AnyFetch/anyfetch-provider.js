 ...
 * @param {Function} next Callback to call once res has been populated.
 */
exports.callback = function(connectAccountRetrieveTempToken, connectAccountRetrieveAuthDatas, cluestrAppId, cluestrAppSecret, connectUrl) {
  return function (req, res, next) {
    async.waterfall([
      function(cb) {
      ...
      function(tempToken, cb) {
        ...
        var cluestrClient = new CluestrClient(cluestrAppId, cluestrAppSecret);
      ...
      function(tempToken, cluestrAccessToken, providerDatas) {
        // Create new token
        var token = new Token({

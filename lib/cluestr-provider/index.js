...
module.exports.validateConfig = function(config) {
  var mandatoryKeys = ['initAccount', 'connectAccountRetrievePreDatas', 'connectAccountRetrieveAuthDatas', 'updateAccount', 'queueWorker', 'cluestrAppId', 'cluestrAppSecret', 'connectUrl'];
...
module.exports.createServer = function(config) {
  ...
  server.get('/init/callback', initEndpoints.callback(config.connectAccountRetrievePreDatas, config.connectAccountRetrieveAuthDatas, cluestrClient, config.connectUrl, config.redirectUrl || 'http://cluestr.com'));

  server.post('/update', updateEndpoints(config.updateAccount, server.queue));

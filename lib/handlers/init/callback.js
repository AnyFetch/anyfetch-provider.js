  ...
  return function(req, res, next) {
    ...
    async.waterfall([
      ...
      function setAccountName(accountName, providerData, tempToken, anyfetchAccessToken, cb) {
        request(process.env.MANAGER_URL || "https://manager.anyfetch.com")
          .post('/access_token/account_name')
          .send({
            access_token: anyfetchAccessToken,
            account_name: accountName
          })
          .expect(204)
          .end(rarity.carryAndSlice([accountName, providerData, tempToken, anyfetchAccessToken], 5, cb));
      },

  ...
  return function(req, res, next) {
    ...
    async.waterfall([
      ...
      function createNewToken(accountName, providerData, tempToken, anyfetchAccessToken, cb) {
        ...
        token.save(rarity.carryAndSlice([anyfetchAccessToken, tempToken], 4, cb));
      },
      function removeTempToken(anyfetchAccessToken, tempToken, cb) {
        tempToken.remove(rarity.carryAndSlice([anyfetchAccessToken], 3, cb));
      },
      ...
      urlObj.query.state = (err instanceof CancelError) ? 'canceled' : 'success';

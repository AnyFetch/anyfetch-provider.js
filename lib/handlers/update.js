    ...
    Token.findOne({anyfetchToken: req.params.access_token}, function(err, token) {
      ...
      async.waterfall([
        ...
        function executeQueue(queues, cb) {
          Object.keys(workers).forEach(function(name) {
            console.log("User `" + token.accountName + "` is pushing " + queues[name].length + " tasks onto `" + name + "`");
            ...
            cconsole.log("User `" + token.accountName + "` have pushed " + queues[name].length + " tasks onto `" + name + "`");
          });
          cb(null);
        }
      ], function(err) {

...
module.exports = function(updateAccount, queue, workers) {
  return function(req, res, next) {
    ...
    Token.findOne({anyfetchToken: req.params.access_token}, function(err, token) {
      ...
      if(!token) {
        console.log("ERR: Unknown token.");
        return next(new restify.InvalidArgumentError("Unknown token."));
      }
      if(token.isUpdating) {
        console.log("ERR: Already processing");
        return next(new restify.InternalError('Already processing'));
      }
      else {
        res.send(202);
      }
      ...
      async.waterfall([
        ...
        function executeQueue(queues, cb) {
          Object.keys(workers).forEach(function(name) {
            console.log("User `" + token.accountName + "` have pushed " + queues[name].length + " tasks onto `" + name + "`");

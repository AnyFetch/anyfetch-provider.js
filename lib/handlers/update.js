...
/**
 * This handler downloads datas for the specified access_token and starts uploading them onto AnyFetch.
 *
...
module.exports = function(updateAccountCb, appId, appSecret, queue) {
  return function(req, res, next) {
    ...
    Token.findOne({anyfetchToken: req.params.access_token}, function(err, token) {
      ...
      if(token.isUpdating && queue.idle()) {
        // If the token is marked as updating but the queue is empty, we force the update
      ...
      async.waterfall([
        ...
        function(cb) {
          ...
          var pushTasksToQueue = function(err, tasks, newCursor) {
            ...
            var anyfetchClient = new AnyFetchClient(appId, appSecret);

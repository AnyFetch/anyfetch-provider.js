    ...
    Token.findOne({anyfetchToken: req.params.access_token}, function(err, token) {
      ...
      if(token.isUpdating) {
        if(Date.now() - token.lastUpdate.getTime() < 5 * 3600 * 1000) {
          ...
          console.warn("WARN: retrying providing after potential crash on " + token.anyfetchToken);
        }
      }
      else {
        res.send(202);
      ...
      async.waterfall([
        ...
        function callUpdateAccount(cb) {
          ...
          updateAccount(token.data || {}, (req.params.ideatifier) ? null : token.cursor, queues, function(err, newCursor, providerData) {

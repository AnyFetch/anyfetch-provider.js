'use strict';
/**
 * @file Update an account from specified access_token
 *
 */

var async = require('async');
var restify = require('restify');
var AnyFetchClient = require('anyfetch');

var Token = require('../models/token.js');


/**
 * This handler downloads data for the specified access_token and starts uploading them onto AnyFetch.
 *
 * @param {Object} req Request object from the client
 * @param {Object} res Response we want to return
 * @param {Function} next Callback to call once res has been populated.
 */
module.exports = function(updateAccountCb, appId, appSecret, queue) {
  return function(req, res, next) {
    if(!req.params.access_token) {
      return next(new restify.MissingParameterError("Missing access_token parameter."));
    }
    if(!req.params.api_url) {
      return next(new restify.MissingParameterError("Missing api_url parameter."));
    }

    Token.findOne({anyfetchToken: req.params.access_token}, function(err, token) {
      if(err) {
        return next(err);
      }
      if(!token) {
        return next(new restify.InvalidArgumentError("Unknown token."));
      }
      if(token.isUpdating && !queue.idle()) {
        res.send(204);
        return next();
      }
      if(token.isUpdating && queue.idle()) {
        // If the token is marked as updating but the queue is empty, we force the update (we had a provider crash or something went wrong, anyway best behavior is to retry)
        res.header('X-Restart-Forced', 'true');
        res.send(202);
        console.log("Restarting update after crash");
      }
      else {
        res.send(202);
      }
      next();


      async.waterfall([
        function(cb) {
          // Update token
          token.lastUpdate = new Date();
          token.isUpdating = true;

          token.save(function(err) {
            cb(err);
          });
        },
        function(cb) {
          var hasEnded = false;

          var pushTasksToQueue = function(err, tasks, newCursor) {
            if(err) {
              return cb(err);
            }

            if(hasEnded) {
              throw new Error("You've sent the new token, and can't queue tasks anymore.");
            }

            if(!tasks || !Array.isArray(tasks)) {
              throw new Error("tasks must be an array");
            }

            var anyfetchClient = new AnyFetchClient(appId, appSecret, req.params.api_url);
            anyfetchClient.setAccessToken(req.params.access_token);

            // Insert the client and data into each task
            tasks.forEach(function(task) {
              task.anyfetchClient = anyfetchClient;
              task.tokenData = token.data;
            });

            queue.push(tasks);

            if(newCursor) {
              hasEnded = true;
              cb(null, newCursor);
            }
          };

          var updateTokenData = function(newData, cb) {
            token.data = newData;
            token.markModified('data');
            token.save(function(err) { cb(err, newData); });
          };

          updateAccountCb(token.data, token.cursor, pushTasksToQueue, updateTokenData);
        },
        function(newCursor, cb) {
          // Insert finalization task
          // The call to this task will save the Token with the new cursor and updates its isUpdating flag.
          queue.push({
            _update: true,
            token: token,
            cursor: newCursor
          });

          cb();
        }
      ], function(err) {
        if(err) {
          console.log('ERR:', err);
          queue.push({
            _update: true,
            token: token,
            failure: true
          });
        }
      });
    });
  };
};

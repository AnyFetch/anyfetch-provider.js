 ...
 */
module.exports.createServer = function createServer(connectFunctions, workers, workersFile, updateFile, config) {
  ...
  server.usersQueue = server.yaqsClient.createQueue('anyfetch-provider-users', {
    concurrency: config.concurrencyUsers || 1,
    defaultTimeout: -1
  });
  ...
  function sigtermYaqs() {
    async.waterfall([
      ...
      function registerUsers(cb) {
        async.each(Object.keys(server.childs), function(anyfetchToken, cb) {
          console.log("SAVE", anyfetchToken);
  ...
  server.usersQueue
    ...
    .setWorker(usersQueue(server, config, workers, workersFile, updateFile))
    .start();

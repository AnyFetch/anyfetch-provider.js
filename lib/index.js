...
module.exports.createServer = function createServer(connectFunctions, updateAccount, workers, config) {
  ...
  var redisUrl = config.redisUrl || process.env.REDIS_URL || "redis://localhost";
  ...
  function sigtermYaqs() {
    server.yaqsClient.stopAllQueues(function(err) {
      console.log('YAQS has stopped successfully.', err || '');
      process.exit(0);
    });
  }
  ...
  server.usersQueue
    ...
    .setWorker(usersQueue(server, config, workers, updateAccount))
    .start();

...
module.exports.createServer = function createServer(connectFunctions, updateAccount, workers, config) {
  ...
  function executeJob(job, done, ctx) {
    async.waterfall([
      ...
      function executeTask(token, cb) {
        ...
        console.log("User `" + token.accountName + "` is executing task " + job.id + " onto `" + job.type + "`");
      ...
      if(err) {
        console.log("Job " + job.id + " onto `" + job.type + "` have failed");
  ...
  server.on('uncaughtException', function(req, res, route, err) {
    console.error('ERR: uncaughtException in restify server', err);

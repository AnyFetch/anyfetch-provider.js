...
module.exports.createServer = function createServer(connectFunctions, updateAccount, workers, config) {
  ...
  config.redisUrl = config.redisUrl || process.env.REDIS_URL || undefined;
  server.queue = kue.createQueue({
    ...
    redis: {
      ...
      auth: (config.redisUrl) ? url.parse(config.redisUrl).auth.split(":")[1] : null,
    },
    disableSearch: true
  });

  // When jobs are completed, we can delete them
  server.queue.on('job complete', function(id) {
  ...
  Object.keys(workers).forEach(function createQueue(name) {
    server.queue.process(name, (workers[name].concurrency) ? workers[name].concurrency : 1, function(job, done) {
      ...
      job.anyfetchClient.setApiUrl(job.task._anyfetchApiUrl);

      function executeTask(token) {
        console.log("User `" + token.accountName + "` is executing task " + job.id + " onto `" + name + "`");

        job.serviceData = token.data;

        delete job.data;
        delete job.task._anyfetchToken;
        delete job.task._anyfetchApiUrl;

        workers[name](job, done);
      }

      if(server.tokenCache.has(job.task._anyfetchToken)) {
        executeTask(server.tokenCache.get(job.task._anyfetchToken));
      }
      else {
        Token.findOne({anyfetchToken: job.task._anyfetchToken}, function(err, token) {
          if(err) {
            return done(err);
          }

          if(!token) {
            return done(new Error("Can't retrieve Token"));
          }


          server.tokenCache.set(job.task._anyfetchToken, token);
          executeTask(token);
        });
      }
    });

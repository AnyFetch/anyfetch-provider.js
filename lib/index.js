...
module.exports.createServer = function createServer(connectFunctions, updateAccount, workers, config) {
  ['appId', 'appSecret', 'providerUrl'].forEach(function(name) {
    if(!config[name]) {
      console.error('Error: Missing config.' + name + ' not found');
      process.exit(1);
    }
  });
  ...
  server.queue.on('job complete', function(id) {
    kue.Job.get(id, function(err, job) {
      if(err && !err.toString().match(/doesnt exist/)) {
  ...
  Object.keys(workers).forEach(function createQueue(name) {
    server.queue.process(name, (workers[name].concurrency) ? workers[name].concurrency : 1, function(job, done, ctx) {
      job.task = job.data;
      ...
      async.waterfall([
        function retrieveToken(cb) {
          if(server.tokenCache.has(job.task._anyfetchToken)) {
            cb(null, server.tokenCache.get(job.task._anyfetchToken));
          }
          else {
            Token.findOne({anyfetchToken: job.task._anyfetchToken}, rarity.slice(2, cb));
          }
        },

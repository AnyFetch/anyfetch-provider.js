...
module.exports.createServer = function createServer(connectFunctions, updateAccount, workers, config) {
  ...
  server.use(require('./middlewares/token.js'));
  server.use(logger);
  ...
  server.post('/update', handlers.update.postGenerator(updateAccount, server.queue, workers));
  server.post('/update/:identifier', handlers.update.postGenerator(updateAccount, server.queue, workers));

  server.get('/status', handlers.status.get);

  server.del('/token', handlers.token.index.del);
  server.del('/token/reset', handlers.token.reset.del);
  ...
  module.exports.createServer.current = server;

      ...
      async.waterfall([
        function retrieveToken(cb) {
          if(server.tokenCache.has(job.data._anyfetchToken)) {
            ...
            Token.findOne({anyfetchToken: job.data._anyfetchToken}, rarity.slice(2, cb));
          }
        },
        function executeTask(token, cb) {
          ...
          if(!token.anyfetchClient) {
            job.anyfetchClient = new Anyfetch(job.data._anyfetchToken);
            job.anyfetchClient.setApiUrl(job.data._anyfetchApiUrl);
          }

          token.anyfetchClient = job.anyfetchClient;

...
module.exports = function userQueueGenerator(config, cache, anyfetchClient, workers, updateAccount) {
  function executeUpdateJob(token, job, cb) {
    async.waterfall([
      ...
      function pushJobsToQueue(queues, cb) {
        async.eachSeries(Object.keys(workers), function(name, cb) {
          // Restrict the maximum number of documents that can be pushed according to the documents_per_update parameter.
          // The limit is applied for each queue. (For Anyfetch's providers, standard queues are addition and deletion)
          ...
          async.eachLimit(queues[name], 5, function(userData, cb) {
            if(documentsCount > job.data.params.documents_per_update || (job.data.params.identifier && userData.identifier !== job.data.params.identifier)) {
              // Call the cb asynchronously.
              // For very large workload, we could get a maximum stack size exceeded.
              // Plus this makes everything faster and avoids hogging down one core.

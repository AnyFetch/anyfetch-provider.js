  ...
  return function usersQueue(job, finalCb) {
    ...
    async.waterfall([
      ...
      function createQueue(token, cb) {
        ...
        console.log("Getting queue for user " + token.accountName);

        queue = server.yaqsClient.createQueue(token.accountName);
        ...
        queue.once('empty', function() {
          queue.removeAllListeners(['job.failed', 'job.completed', 'job.timeout']);
          queue.remove(finalCb);
        });

        cb(null, token);
      },
      function startQueue(token, cb) {

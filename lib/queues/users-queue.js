  ...
  return function usersQueue(job, finalCb) {
    ...
    async.waterfall([
      ...
      function createQueue(token, cb) {
        if(!token) {
          // Token was probably revoked or not valid anymore
          finalCb();
          return cb(null);
        }
        ...
        childs.on('stop', function(forced) {
          ...
          queue.remove(function(err) {
            ...
            anyfetchClient.trackProviderUpdate({
              ...
              time_spent_on_tasks: new Date() - dateStartOfTasks,
        ...
        childs.on('message', function(data) {
          if(data.type === 'event') {
            if(data.event === 'failed') {
              ...
              return anyfetchClient.refreshToken(function(err) {
                if(err) {
                  util.logError(err, data.job);
                }

                childs.stop();

                token.requireRefresh = true;
                token.save(util.logError);
              });
            }
